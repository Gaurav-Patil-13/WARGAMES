...
export async function listLevelsForUser(userId) {
  ...
  const result = await db.query(`
    SELECT l.id, l.slug, l.title, l.track, l.xp, l.image, l.objective, l.hint, l.walkthrough,
           ...
           p.completed_at
    FROM levels l
    LEFT JOIN progress p ON p.level_id = l.id AND p.user_id = $1
    ORDER BY l.id
  `, [userId]);
...
export async function recordAttempt(userId, levelId) {
  ...
  await db.query(`
    ...
    DO UPDATE SET attempts = progress.attempts + 1
  `, [userId, levelId]);
}
...
export async function completeLevel(userId, levelId, xp) {
  ...
  await db.query(`
    ...
    VALUES (
      ...
      $2,
      1,
      COALESCE((SELECT attempts FROM progress WHERE user_id = $3 AND level_id = $4), 0),
      CURRENT_TIMESTAMP
    )
    ON CONFLICT(user_id, level_id)
    DO UPDATE SET
